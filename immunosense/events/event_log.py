...
class EventLog:
    ...
    The store is keyed by (patient_id, date). All reads and writes go
    ...
    def _patient_dir(self, patient_id: str) -> Path:
        d = self.root / patient_id
        d.mkdir(parents=True, exist_ok=True)
        return d

    def _file_for(self, patient_id: str, date: str) -> Path:
        """Path to the NDJSON file for one patient-day."""
        return self._patient_dir(patient_id) / f"{date}.ndjson"
    ...
    def append(self, event: Event) -> None:
        ...
        path = self._file_for(event.patient_id, self._date_of(event))
    ...
    def append_many(self, events: Iterable[Event]) -> int:
        ...
        for ev in events:
            by_file[self._file_for(ev.patient_id, self._date_of(ev))].append(ev)
    ...
    # Read
    # ------------------------------------------------------------------ #
    def read_day(self, patient_id: str, date: str) -> list:
        """Read all events for one patient-day, in file (append) order.

        Returns an empty list if the file doesn't exist.
        """
        path = self._file_for(patient_id, date)
        if not path.exists():
            return []
        events = []
    ...
    def read_range(
        ...
        cursor = start
        while cursor <= end:
            events.extend(self.read_day(patient_id, cursor.strftime("%Y-%m-%d")))
            cursor += timedelta(days=1)
        events.sort(key=lambda e: e.timestamp)
        return events

    def read_bucket(self, patient_id: str, bucket_id: str) -> list:
        ...
        date = parts[1]
        return [e for e in self.read_day(patient_id, date) if e.bucket_id == bucket_id]
    ...
    def count(self, patient_id: str) -> int:
        ...
        return sum(1 for _ in self.iter_events(patient_id))

    def patients(self) -> list:
        """List patient_ids that have any events on disk."""
